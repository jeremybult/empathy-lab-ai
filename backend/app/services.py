...
class ChatService:
    ...
    def chat(self, client_id: str, persona_id: str, message: str) -> dict:
        ...
        system_message = {
            ...
            'content': (
                ...
                + ' Preserve the selected persona voice and premise.'
        ...
        try:
            ...
            logger.exception('OpenAI request transport failure')
            raise UpstreamProviderError('Upstream provider request failed') from exc

        if response.status_code >= 400:
            logger.error(
                'OpenAI API non-success status=%s body=%s',
                response.status_code,
                response.text,
            )
            raise UpstreamProviderError(
                'Upstream provider returned non-success status',
        ...
        try:
            ...
            logger.exception('Unexpected OpenAI response payload shape')
            raise UpstreamProviderError('Upstream provider returned invalid payload') from exc
    ...
    def _demo_reply(self, persona: dict, user_message: str) -> str:
        ...
        origin = persona.get('origin', 'modern')

        if 'hello' in lower or 'hi' in lower:
            return (
                f'Hi, I am {name}. This is demo mode, but the backend flow is working. '
                f'I am loaded as a {origin} persona. '
                'Ask me about symptoms, lived experience, study habits, character roleplay, or how this project supports empathy development.'
            )
        if 'adhd' in lower:
            return (
        ...
        if 'bee' in lower and persona['id'] == 'bee_soldier':
            return (
                'Bzzzz, Rock here. The war with the wasps is fierce, but nature is nature. '
                'The Titan has not returned in some time, so we keep flying and holding the line.'
            )
        if 'green' in lower and persona['id'] == 'hammond_cover':
            return (
                'Green is still one of my favorite colors, though I could not honestly tell you what it looks like. '
                'Funny thing to be known for, but audiences seem to enjoy it.'
            )
        if 'dark' in lower and persona['id'] == 'pajama_sam_legacy':
            return (
                'Babaga-BOOSH. The dark is definitely spooky, but that does not mean it wins. '
                'Sometimes the trick is being brave long enough to find the flashlight first.'
            )
        if 'empathy' in lower:
            return (
        ...
        return (
            ...
            'Once you add a key, this same interface will return live model responses while keeping the free browser-based voice path and preserving legacy persona support.'
        )
