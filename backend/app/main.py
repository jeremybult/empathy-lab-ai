...
from .services import ChatService, UpstreamProviderError
from .settings import get_settings
from .storage import MemoryConversationStore
from .voice import VoiceProviderService
...
voice_service = VoiceProviderService(settings=settings)
...
def voice_providers() -> dict:
    return voice_service.get_provider_status()
...
def chat(request: Request, payload: ChatRequest, response: Response) -> ChatResponse:
    ...
    try:
        ...
        logger.exception('Upstream provider error during /api/chat')
        raise HTTPException(
            status_code=502,
            detail='The AI provider is temporarily unavailable. Please try again shortly.',
        ) from exc
    except Exception as exc:
        logger.exception('Unexpected internal error during /api/chat')
        raise HTTPException(
            status_code=500,
            detail='Internal server error. Please try again later.',
        ) from exc

    set_session_cookie(response=response, client_id=client_id)
